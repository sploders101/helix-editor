use super::{
    tree::{TreeOp, TreeView, TreeViewItem},
    Prompt,
};
use crate::{
...
impl Explorer {
    ...
    pub fn column_width(&self, cx: &mut Context) -> u16 {
        self.column_width
            .unwrap_or(cx.editor.config().explorer.column_width as u16)
    }

    fn increase_size(&mut self, cx: &mut Context) {
        self.column_width = Some(self.column_width(cx).saturating_add(1))
    }

    fn decrease_size(&mut self, cx: &mut Context) {
        self.column_width = Some(self.column_width(cx).saturating_sub(1))
    }
...
fn close_documents(current_item_path: PathBuf, cx: &mut Context) -> Result<()> {
    ...
    for id in ids {
        // TODO: shaunkeys: handle this properly
        match cx.editor.close_document(id, true) {
            Ok(()) => {}
            Err(CloseError::DoesNotExist) => anyhow::bail!("Document doesn't exist"),
            Err(CloseError::BufferModified(err)) => anyhow::bail!("Buffer modified: {err}"),
            Err(CloseError::SaveError(err)) => anyhow::bail!("Save error: {err}"),
        };
    }
    Ok(())
}
    ...
    fn handle_event(&mut self, event: &Event, cx: &mut Context) -> EventResult {
        ...
        (|| -> Result<()> {
            match key_event {
                ...
                key!('-') | key!('_') => self.decrease_size(cx),
                key!('+') | key!('=') => self.increase_size(cx),
                _ => {
                    self.tree
